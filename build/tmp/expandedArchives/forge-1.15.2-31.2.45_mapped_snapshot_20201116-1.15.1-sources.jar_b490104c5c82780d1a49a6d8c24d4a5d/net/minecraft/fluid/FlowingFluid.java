...
public abstract class FlowingFluid extends Fluid {
   ...
   public Vec3d getFlow(IBlockReader blockReader, BlockPos pos, IFluidState fluidStateIn) {
      double d0 = 0.0D;
      double d1 = 0.0D;
      ...
      try (BlockPos.PooledMutable blockpos$pooledmutable = BlockPos.PooledMutable.retain()) {
         for(Direction direction : Direction.Plane.HORIZONTAL) {
            ...
            IFluidState ifluidstate = blockReader.getFluidState(blockpos$pooledmutable);
            if (this.isSameOrEmpty(ifluidstate)) {
               float f = ifluidstate.getHeight();
               float f1 = 0.0F;
               if (f == 0.0F) {
                  if (!blockReader.getBlockState(blockpos$pooledmutable).getMaterial().blocksMovement()) {
                     BlockPos blockpos = blockpos$pooledmutable.down();
                     IFluidState ifluidstate1 = blockReader.getFluidState(blockpos);
                     if (this.isSameOrEmpty(ifluidstate1)) {
                        f = ifluidstate1.getHeight();
                        if (f > 0.0F) {
                           f1 = fluidStateIn.getHeight() - (f - 0.8888889F);
                        }
                     }
                  }
               } else if (f > 0.0F) {
                  f1 = fluidStateIn.getHeight() - f;
               }
         ...
         if (fluidStateIn.get(FALLING)) {
            for(Direction direction1 : Direction.Plane.HORIZONTAL) {
               blockpos$pooledmutable.setPos(pos).move(direction1);
               if (this.causesDownwardCurrent(blockReader, blockpos$pooledmutable, direction1) || this.causesDownwardCurrent(blockReader, blockpos$pooledmutable.up(), direction1)) {
