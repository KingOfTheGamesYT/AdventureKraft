...
public class ChunkManager extends ChunkLoader implements ChunkHolder.IPlayerProvider {
   ...
   public CompletableFuture<Either<IChunk, ChunkHolder.IChunkLoadingError>> func_219244_a(ChunkHolder chunkHolderIn, ChunkStatus chunkStatusIn) {
      ChunkPos chunkpos = chunkHolderIn.getPosition();
      if (chunkStatusIn == ChunkStatus.EMPTY) {
         return this.chunkLoad(chunkpos);
      } else {
         CompletableFuture<Either<IChunk, ChunkHolder.IChunkLoadingError>> completablefuture = chunkHolderIn.func_219276_a(chunkStatusIn.getParent(), this);
            ...
            if (!optional.isPresent()) {
               ...
               if (ichunk.getStatus().isAtLeast(chunkStatusIn)) {
                  ...
                  if (chunkStatusIn == ChunkStatus.LIGHT) {
                     ...
                     completablefuture1 = chunkStatusIn.doLoadingWork(this.world, this.templateManager, this.lightManager, (p_223175_2_) -> {
                        return this.func_219200_b(chunkHolderIn);
                     }, ichunk);
                  }

                  this.field_219266_t.statusChanged(chunkpos, chunkStatusIn);
   ...
   private CompletableFuture<Either<IChunk, ChunkHolder.IChunkLoadingError>> chunkLoad(ChunkPos chunkPosIn) {
      return CompletableFuture.supplyAsync(() -> {
         try {
            this.world.getProfiler().func_230035_c_("chunkLoad");
            CompoundNBT compoundnbt = this.loadChunkData(chunkPosIn);
            if (compoundnbt != null) {
               boolean flag = compoundnbt.contains("Level", 10) && compoundnbt.getCompound("Level").contains("Status", 8);
               if (flag) {
                  IChunk ichunk = ChunkSerializer.read(this.world, this.templateManager, this.pointOfInterestManager, chunkPosIn, compoundnbt);
                  ichunk.setLastSaveTime(this.world.getGameTime());
                  return Either.left(ichunk);
               }

               LOGGER.error("Chunk file at {} is missing level data, skipping", (Object)chunkPosIn);
            }
            ...
            LOGGER.error("Couldn't load chunk {}", chunkPosIn, throwable);
         } catch (Exception exception) {
            LOGGER.error("Couldn't load chunk {}", chunkPosIn, exception);
         }

         return Either.left(new ChunkPrimer(chunkPosIn, UpgradeData.EMPTY));
      }, this.mainThread);
   }

   private CompletableFuture<Either<IChunk, ChunkHolder.IChunkLoadingError>> chunkGenerate(ChunkHolder chunkHolderIn, ChunkStatus chunkStatusIn) {
      ChunkPos chunkpos = chunkHolderIn.getPosition();
      CompletableFuture<Either<List<IChunk>, ChunkHolder.IChunkLoadingError>> completablefuture = this.func_219236_a(chunkpos, chunkStatusIn.getTaskRange(), (p_219195_2_) -> {
         return this.func_219205_a(chunkStatusIn, p_219195_2_);
      });
      ...
      return completablefuture.thenComposeAsync((p_219235_4_) -> {
         return p_219235_4_.map((p_223148_4_) -> {
            try {
               CompletableFuture<Either<IChunk, ChunkHolder.IChunkLoadingError>> completablefuture1 = chunkStatusIn.doGenerationWork(this.world, this.generator, this.templateManager, this.lightManager, (p_222954_2_) -> {
                  return this.func_219200_b(chunkHolderIn);
               }, p_223148_4_);
               this.field_219266_t.statusChanged(chunkpos, chunkStatusIn);
               return completablefuture1;
         ...
         this.field_219264_r.enqueue(ChunkTaskPriorityQueueSorter.func_219081_a(chunkHolderIn, p_219216_2_));
      });
   }
   ...
   protected void sendToAllTracking(Entity entityIn, IPacket<?> p_219222_2_) {
      ChunkManager.EntityTracker chunkmanager$entitytracker = this.entities.get(entityIn.getEntityId());
   ...
   protected void sendToTrackingAndSelf(Entity entityIn, IPacket<?> p_219225_2_) {
      ChunkManager.EntityTracker chunkmanager$entitytracker = this.entities.get(entityIn.getEntityId());
      if (chunkmanager$entitytracker != null) {
   ...
   public CompletableFuture<Void> func_222973_a(Chunk chunkIn) {
      return this.mainThread.runAsync(() -> {
         chunkIn.saveScheduledTicks(this.world);
      });
   }
   ...
   class EntityTracker {
      ...
      public EntityTracker(Entity entityIn, int p_i50468_3_, int updateFrequency, boolean sendVelocityUpdatesIn) {
         this.entry = new TrackedEntity(ChunkManager.this.world, entityIn, updateFrequency, sendVelocityUpdatesIn, this::sendToAllTracking);
         this.entity = entityIn;
         this.range = p_i50468_3_;
         this.pos = SectionPos.from(entityIn);
      }
      ...
      public void updateTrackingState(ServerPlayerEntity player) {
         if (player != this.entity) {
            ...
            boolean flag = vec3d.x >= (double)(-i) && vec3d.x <= (double)i && vec3d.z >= (double)(-i) && vec3d.z <= (double)i && this.entity.isSpectatedByPlayer(player);
            if (flag) {
      ...
      public void updateTrackingState(List<ServerPlayerEntity> playersListIn) {
         for(ServerPlayerEntity serverplayerentity : playersListIn) {
