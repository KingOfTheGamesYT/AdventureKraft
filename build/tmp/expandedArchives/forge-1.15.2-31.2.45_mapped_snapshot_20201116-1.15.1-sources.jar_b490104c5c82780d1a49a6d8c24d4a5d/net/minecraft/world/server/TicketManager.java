...
public abstract class TicketManager {
   ...
   public boolean processUpdates(ChunkManager chunkManagerIn) {
      ...
      if (!this.chunkHolders.isEmpty()) {
         this.chunkHolders.forEach((p_219343_1_) -> {
            p_219343_1_.processUpdates(chunkManagerIn);
         });
         ...
         if (!this.chunkPositions.isEmpty()) {
            ...
            while(longiterator.hasNext()) {
               ...
               if (this.getTicketSet(j).stream().anyMatch((p_219369_0_) -> {
                  ...
                  ChunkHolder chunkholder = chunkManagerIn.func_219220_a(j);
                  if (chunkholder == null) {
   ...
   class PlayerChunkTracker extends ChunkDistanceGraph {
      ...
      protected PlayerChunkTracker(int levelCountIn) {
         super(levelCountIn + 2, 16, 256);
         this.range = levelCountIn;
         this.chunksInRange.defaultReturnValue((byte)(levelCountIn + 2));
      }
