...
public class SkyLightStorage extends SectionLightStorage<SkyLightStorage.StorageMap> {
   ...
   protected void addSection(long sectionPosIn) {
      int i = SectionPos.extractY(sectionPosIn);
      ...
      long j = SectionPos.toSectionColumnPos(sectionPosIn);
      ...
      if (k < i + 1) {
         ...
         if (this.enabledColumns.contains(j)) {
            this.scheduleFullUpdate(sectionPosIn);
            if (k > (this.cachedLightData).minY) {
               long l = SectionPos.asLong(SectionPos.extractX(sectionPosIn), k - 1, SectionPos.extractZ(sectionPosIn));
