...
public class Chunk extends net.minecraftforge.common.capabilities.CapabilityProvider<Chunk> implements IChunk, net.minecraftforge.common.extensions.IForgeChunk {
   ...
   public Chunk(World worldIn, ChunkPrimer chunkPrimerIn) {
      this(worldIn, chunkPrimerIn.getPos(), chunkPrimerIn.getBiomes(), chunkPrimerIn.getUpgradeData(), chunkPrimerIn.getBlocksToBeTicked(), chunkPrimerIn.getFluidsToBeTicked(), chunkPrimerIn.getInhabitedTime(), chunkPrimerIn.getSections(), (Consumer<Chunk>)null);

      for(CompoundNBT compoundnbt : chunkPrimerIn.getEntities()) {
      ...
      for(TileEntity tileentity : chunkPrimerIn.getTileEntities().values()) {
         this.addTileEntity(tileentity);
      }

      this.deferredTileEntities.putAll(chunkPrimerIn.getDeferredTileEntities());

      for(int i = 0; i < chunkPrimerIn.getPackedPositions().length; ++i) {
         this.packedBlockPositions[i] = chunkPrimerIn.getPackedPositions()[i];
      }

      this.setStructureStarts(chunkPrimerIn.getStructureStarts());
      this.setStructureReferences(chunkPrimerIn.getStructureReferences());

      for(Entry<Heightmap.Type, Heightmap> entry : chunkPrimerIn.getHeightmaps()) {
      ...
      this.setLight(chunkPrimerIn.hasLight());
      this.dirty = true;
   }
