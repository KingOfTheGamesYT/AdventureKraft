...
public class BucketItem extends Item {
   ...
   protected ItemStack emptyBucket(ItemStack stack, PlayerEntity playerIn) {
      return !playerIn.abilities.isCreativeMode ? new ItemStack(Items.BUCKET) : stack;
   }
   ...
   public boolean tryPlaceContainedLiquid(@Nullable PlayerEntity player, World worldIn, BlockPos posIn, @Nullable BlockRayTraceResult rayTrace) {
      if (!(this.containedBlock instanceof FlowingFluid)) {
         ...
         if (blockstate.isAir() || flag || canContainFluid) {
            ...
            return rayTrace == null ? false : this.tryPlaceContainedLiquid(player, worldIn, rayTrace.getPos().offset(rayTrace.getFace()), (BlockRayTraceResult)null);
         }
      }
   }
