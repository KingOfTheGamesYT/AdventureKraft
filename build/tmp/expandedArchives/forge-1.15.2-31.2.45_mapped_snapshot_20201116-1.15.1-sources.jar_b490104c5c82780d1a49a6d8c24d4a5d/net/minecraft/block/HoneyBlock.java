...
public class HoneyBlock extends BreakableBlock {
   ...
   private boolean func_226935_a_(BlockPos pos, Entity entityIn) {
   ...
   private void func_226933_a_(Entity entityIn, BlockPos pos) {
   ...
   private void func_226938_d_(Entity entityIn) {
      ...
      if (vec3d.y < -0.13D) {
         ...
         entityIn.setMotion(new Vec3d(vec3d.x * d0, -0.05D, vec3d.z * d0));
      } else {
         entityIn.setMotion(new Vec3d(vec3d.x, -0.05D, vec3d.z));
      }

      entityIn.fallDistance = 0.0F;
   }

   private void func_226934_a_(World worldIn, Entity entityIn) {
      if (func_226937_c_(entityIn)) {
         if (worldIn.rand.nextInt(5) == 0) {
            entityIn.playSound(SoundEvents.BLOCK_HONEY_BLOCK_SLIDE, 1.0F, 1.0F);
         }

         if (!worldIn.isRemote && worldIn.rand.nextInt(5) == 0) {
            worldIn.setEntityState(entityIn, (byte)53);
         }
      }
   ...
   @OnlyIn(Dist.CLIENT)
   public static void func_226931_a_(Entity entityIn) {
      func_226932_a_(entityIn, 5);
   }

   @OnlyIn(Dist.CLIENT)
   public static void func_226936_b_(Entity entityIn) {
      func_226932_a_(entityIn, 10);
   }

   @OnlyIn(Dist.CLIENT)
   private static void func_226932_a_(Entity entityIn, int p_226932_1_) {
