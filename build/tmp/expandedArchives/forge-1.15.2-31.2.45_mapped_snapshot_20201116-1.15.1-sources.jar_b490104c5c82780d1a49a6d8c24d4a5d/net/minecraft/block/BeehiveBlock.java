...
public class BeehiveBlock extends ContainerBlock {
   ...
   public static void dropHoneyComb(World worldIn, BlockPos posIn) {
      spawnAsEntity(worldIn, posIn, new ItemStack(Items.HONEYCOMB, 3));
   }
   ...
   private boolean hasBees(World worldIn, BlockPos posIn) {
   ...
   public void takeHoney(World worldIn, BlockState state, BlockPos posIn, @Nullable PlayerEntity playerIn, BeehiveTileEntity.State stateIn) {
      this.takeHoney(worldIn, state, posIn);
      TileEntity tileentity = worldIn.getTileEntity(posIn);
      if (tileentity instanceof BeehiveTileEntity) {
         BeehiveTileEntity beehivetileentity = (BeehiveTileEntity)tileentity;
         beehivetileentity.angerBees(playerIn, state, stateIn);
      }

   }

   public void takeHoney(World worldIn, BlockState state, BlockPos posIn) {
      worldIn.setBlockState(posIn, state.with(HONEY_LEVEL, Integer.valueOf(0)), 3);
   }
   ...
   @OnlyIn(Dist.CLIENT)
   private void func_226879_a_(World worldIn, BlockPos pos, BlockState stateIn) {
      if (stateIn.getFluidState().isEmpty() && !(worldIn.rand.nextFloat() < 0.3F)) {
         VoxelShape voxelshape = stateIn.getCollisionShape(worldIn, pos);
         double d0 = voxelshape.getEnd(Direction.Axis.Y);
         if (d0 >= 1.0D && !stateIn.isIn(BlockTags.IMPERMEABLE)) {
            double d1 = voxelshape.getStart(Direction.Axis.Y);
            if (d1 > 0.0D) {
               this.addHoneyParticle(worldIn, pos, voxelshape, (double)pos.getY() + d1 - 0.05D);
            } else {
               BlockPos blockpos = pos.down();
               BlockState blockstate = worldIn.getBlockState(blockpos);
               VoxelShape voxelshape1 = blockstate.getCollisionShape(worldIn, blockpos);
               double d2 = voxelshape1.getEnd(Direction.Axis.Y);
               if ((d2 < 1.0D || !blockstate.isCollisionShapeOpaque(worldIn, blockpos)) && blockstate.getFluidState().isEmpty()) {
                  this.addHoneyParticle(worldIn, pos, voxelshape, (double)pos.getY() - 0.05D);
               }
            }
         }
   ...
   @OnlyIn(Dist.CLIENT)
   private void addHoneyParticle(World worldIn, BlockPos posIn, VoxelShape shapeIn, double p_226880_4_) {
      this.addHoneyParticle(worldIn, (double)posIn.getX() + shapeIn.getStart(Direction.Axis.X), (double)posIn.getX() + shapeIn.getEnd(Direction.Axis.X), (double)posIn.getZ() + shapeIn.getStart(Direction.Axis.Z), (double)posIn.getZ() + shapeIn.getEnd(Direction.Axis.Z), p_226880_4_);
   }
