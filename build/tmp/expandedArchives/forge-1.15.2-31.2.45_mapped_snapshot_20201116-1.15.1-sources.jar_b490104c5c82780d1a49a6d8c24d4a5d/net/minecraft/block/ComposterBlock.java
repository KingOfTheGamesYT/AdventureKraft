...
public class ComposterBlock extends Block implements ISidedInventoryProvider {
   ...
   private static final VoxelShape[] SHAPE = Util.make(new VoxelShape[9], (p_220291_0_) -> {
      for(int i = 0; i < 8; ++i) {
         p_220291_0_[i] = VoxelShapes.combineAndSimplify(OUT_SHAPE, Block.makeCuboidShape(2.0D, (double)Math.max(2, 1 + i * 2), 2.0D, 14.0D, 16.0D, 14.0D), IBooleanFunction.ONLY_FIRST);
      }
   ...
   @OnlyIn(Dist.CLIENT)
   public static void playEvent(World worldIn, BlockPos pos, boolean p_220292_2_) {
      BlockState blockstate = worldIn.getBlockState(pos);
      worldIn.playSound((double)pos.getX(), (double)pos.getY(), (double)pos.getZ(), p_220292_2_ ? SoundEvents.BLOCK_COMPOSTER_FILL_SUCCESS : SoundEvents.BLOCK_COMPOSTER_FILL, SoundCategory.BLOCKS, 1.0F, 1.0F, false);
      double d0 = blockstate.getShape(worldIn, pos).max(Direction.Axis.Y, 0.5D, 0.5D) + 0.03125D;
      ...
      for(int i = 0; i < 10; ++i) {
         ...
         worldIn.addParticle(ParticleTypes.COMPOSTER, (double)pos.getX() + (double)0.13125F + (double)0.7375F * (double)random.nextFloat(), (double)pos.getY() + d0 + (double)random.nextFloat() * (1.0D - d0), (double)pos.getZ() + (double)0.13125F + (double)0.7375F * (double)random.nextFloat(), d3, d4, d5);
      }
