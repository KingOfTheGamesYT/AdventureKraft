...
public class CampfireBlock extends ContainerBlock implements IWaterLoggable {
   ...
   public static boolean isLitCampfireInRange(World worldIn, BlockPos pos, int p_226914_2_) {
      for(int i = 1; i <= p_226914_2_; ++i) {
         ...
         boolean flag = VoxelShapes.compare(field_226912_f_, blockstate.getCollisionShape(worldIn, pos, ISelectionContext.dummy()), IBooleanFunction.AND);
         if (flag) {
   ...
   private static boolean func_226915_i_(BlockState state) {
      return state.getBlock() == Blocks.CAMPFIRE && state.get(LIT);
   }
