...
public class WitherSkeletonSkullBlock extends SkullBlock {
   ...
   public static void checkWitherSpawn(World worldIn, BlockPos pos, SkullTileEntity tileEntityIn) {
      if (!worldIn.isRemote) {
   ...
   public static boolean canSpawnMob(World worldIn, BlockPos pos, ItemStack stack) {
      if (stack.getItem() == Items.WITHER_SKELETON_SKULL && pos.getY() >= 2 && worldIn.getDifficulty() != Difficulty.PEACEFUL && !worldIn.isRemote) {
         return getOrCreateWitherBase().match(worldIn, pos) != null;
      } else {
         return false;
      }
   }
