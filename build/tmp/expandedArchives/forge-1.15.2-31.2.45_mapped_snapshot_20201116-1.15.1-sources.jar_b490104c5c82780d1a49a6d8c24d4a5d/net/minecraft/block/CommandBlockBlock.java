...
public class CommandBlockBlock extends ContainerBlock {
   ...
   private static void executeChain(World worldIn, BlockPos pos, Direction directionIn) {
      BlockPos.Mutable blockpos$mutable = new BlockPos.Mutable(pos);
      GameRules gamerules = worldIn.getGameRules();

      int i;
      BlockState blockstate;
      for(i = gamerules.getInt(GameRules.MAX_COMMAND_CHAIN_LENGTH); i-- > 0; directionIn = blockstate.get(FACING)) {
         blockpos$mutable.move(directionIn);
         blockstate = worldIn.getBlockState(blockpos$mutable);
         Block block = blockstate.getBlock();
         if (block != Blocks.CHAIN_COMMAND_BLOCK) {
            break;
         }

         TileEntity tileentity = worldIn.getTileEntity(blockpos$mutable);
         if (!(tileentity instanceof CommandBlockTileEntity)) {
            break;
         }
         ...
         if (commandblocktileentity.isPowered() || commandblocktileentity.isAuto()) {
            ...
            if (commandblocktileentity.setConditionMet()) {
               if (!commandblocklogic.trigger(worldIn)) {
                  break;
               }

               worldIn.updateComparatorOutputLevel(blockpos$mutable, block);
            } else if (commandblocktileentity.isConditional()) {
               commandblocklogic.setSuccessCount(0);
            }
         }
