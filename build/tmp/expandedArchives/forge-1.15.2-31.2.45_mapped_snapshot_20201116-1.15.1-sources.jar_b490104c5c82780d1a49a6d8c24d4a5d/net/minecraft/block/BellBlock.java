...
public class BellBlock extends ContainerBlock {
    ...
    */
   private boolean canRingFrom(BlockState pos, Direction directionIn, double p_220129_3_) {
      if (directionIn.getAxis() != Direction.Axis.Y && !(p_220129_3_ > (double)0.8124F)) {
   ...
   public boolean func_226885_a_(World worldIn, BlockPos pos, @Nullable Direction directionIn) {
      TileEntity tileentity = worldIn.getTileEntity(pos);
      if (!worldIn.isRemote && tileentity instanceof BellTileEntity) {
         if (directionIn == null) {
            directionIn = worldIn.getBlockState(pos).get(HORIZONTAL_FACING);
         }

         ((BellTileEntity)tileentity).ring(directionIn);
