...
public class LecternBlock extends ContainerBlock {
   ...
   private void dropBook(BlockState state, World worldIn, BlockPos pos) {
      ...
      if (tileentity instanceof LecternTileEntity) {
         ...
         ItemEntity itementity = new ItemEntity(worldIn, (double)pos.getX() + 0.5D + (double)f, (double)(pos.getY() + 1), (double)pos.getZ() + 0.5D + (double)f1, itemstack);
   ...
   private void openContainer(World worldIn, BlockPos pos, PlayerEntity playerIn) {
      TileEntity tileentity = worldIn.getTileEntity(pos);
      if (tileentity instanceof LecternTileEntity) {
         playerIn.openContainer((LecternTileEntity)tileentity);
