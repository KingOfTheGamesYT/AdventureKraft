...
public class HopperContainer extends Container {
   ...
   public HopperContainer(int id, PlayerInventory playerInventoryIn) {
      this(id, playerInventoryIn, new Inventory(5));
   }

   public HopperContainer(int id, PlayerInventory playerInventoryIn, IInventory inventoryIn) {
      super(ContainerType.HOPPER, id);
      this.hopperInventory = inventoryIn;
      assertInventorySize(inventoryIn, 5);
      inventoryIn.openInventory(playerInventoryIn.player);
      int i = 51;

      for(int j = 0; j < 5; ++j) {
         this.addSlot(new Slot(inventoryIn, j, 44 + j * 18, 20));
      }

      for(int l = 0; l < 3; ++l) {
         for(int k = 0; k < 9; ++k) {
            this.addSlot(new Slot(playerInventoryIn, k + l * 9 + 9, 8 + k * 18, l * 18 + 51));
         }
      }

      for(int i1 = 0; i1 < 9; ++i1) {
         this.addSlot(new Slot(playerInventoryIn, i1, 8 + i1 * 18, 109));
      }
