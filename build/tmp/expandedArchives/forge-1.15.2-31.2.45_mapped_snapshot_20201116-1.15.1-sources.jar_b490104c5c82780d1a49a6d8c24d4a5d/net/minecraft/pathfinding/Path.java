...
public class Path {
   ...
   @OnlyIn(Dist.CLIENT)
   private Set<FlaggedPathPoint> targetPointSet;
   private int currentPathIndex;
   private final BlockPos target;
   private final float manhattanLength;
   ...
   public Path(List<PathPoint> pointsIn, BlockPos targetIn, boolean reachesTargetIn) {
      ...
      this.manhattanLength = pointsIn.isEmpty() ? Float.MAX_VALUE : this.points.get(this.points.size() - 1).manhattanDistanceTo(this.target);
   ...
   public static Path read(PacketBuffer buf) {
      ...
      for(int k = 0; k < j; ++k) {
         set.add(FlaggedPathPoint.deserializeFromBuffer(buf));
      }
   ...
   /**
    * returns the manhattan distance from the first to the last path point
    */
