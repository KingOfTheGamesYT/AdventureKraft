...
public class PathPoint {
   ...
   /** The total euclidean distance from the start point to this path point. */
   ...
   public PathPoint cloneMove(int x, int y, int z) {
      ...
      pathpoint.pathLength = this.pathLength;
   ...
   /**
    * returns the manhattan distance to another path point
    */
   public float manhattanDistanceTo(PathPoint pathpointIn) {
      float f = (float)Math.abs(pathpointIn.x - this.x);
      float f1 = (float)Math.abs(pathpointIn.y - this.y);
      float f2 = (float)Math.abs(pathpointIn.z - this.z);
      return f + f1 + f2;
   }

   /**
    * returns the manhattan distance to a BlockPos
    */
   public float manhattanDistanceTo(BlockPos pos) {
   ...
   public static PathPoint createFromBuffer(PacketBuffer buf) {
      ...
      pathpoint.pathLength = buf.readFloat();
