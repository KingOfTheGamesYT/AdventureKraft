...
public abstract class VoxelShape {
   ...
   @Nullable
   public BlockRayTraceResult rayTrace(Vec3d startVec, Vec3d endVec, BlockPos pos) {
      if (this.isEmpty()) {
         return null;
      } else {
         Vec3d vec3d = endVec.subtract(startVec);
         if (vec3d.lengthSquared() < 1.0E-7D) {
            return null;
         } else {
            Vec3d vec3d1 = startVec.add(vec3d.scale(0.001D));
            return this.contains(vec3d1.x - (double)pos.getX(), vec3d1.y - (double)pos.getY(), vec3d1.z - (double)pos.getZ()) ? new BlockRayTraceResult(vec3d1, Direction.getFacingFromVector(vec3d.x, vec3d.y, vec3d.z).getOpposite(), pos, true) : AxisAlignedBB.rayTrace(this.toBoundingBoxList(), startVec, endVec, pos);
         }
      }
   }
