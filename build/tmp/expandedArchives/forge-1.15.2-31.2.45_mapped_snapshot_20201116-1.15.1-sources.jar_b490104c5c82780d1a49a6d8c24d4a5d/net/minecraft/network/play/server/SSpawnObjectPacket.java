...
public class SSpawnObjectPacket implements IPacket<IClientPlayNetHandler> {
   ...
   public SSpawnObjectPacket(int entityIdIn, UUID uuid, double xPos, double yPos, double zPos, float pitchIn, float yawIn, EntityType<?> entityTypeIn, int entityData, Vec3d speedVector) {
      this.entityId = entityIdIn;
      this.uniqueId = uuid;
      this.x = xPos;
      this.y = yPos;
      this.z = zPos;
      this.pitch = MathHelper.floor(pitchIn * 256.0F / 360.0F);
      this.yaw = MathHelper.floor(yawIn * 256.0F / 360.0F);
   ...
   public SSpawnObjectPacket(Entity entityIn) {
      this(entityIn, 0);
   }
   ...
   public SSpawnObjectPacket(Entity entityIn, EntityType<?> entityTypeIn, int entityData, BlockPos pos) {
      this(entityIn.getEntityId(), entityIn.getUniqueID(), (double)pos.getX(), (double)pos.getY(), (double)pos.getZ(), entityIn.rotationPitch, entityIn.rotationYaw, entityTypeIn, entityData, entityIn.getMotion());
   }
