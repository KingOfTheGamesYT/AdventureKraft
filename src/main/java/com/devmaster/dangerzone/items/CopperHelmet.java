...
public class CopperHelmet extends ArmorItem {
    private static IArmorMaterial iArmorMaterial = new IArmorMaterial() {
        ...
        public int getDurability(EquipmentSlotType slotIn) {
            return (Integer) DZConfig.COPPER_HELMET_DURABILITY.get();
        }

        @Override
        public int getDamageReductionAmount(EquipmentSlotType slotIn) {
            return (Integer) DZConfig.COPPER_HELMET_ARMOR.get();
        }

        @Override
        public int getEnchantability() {
            return (Integer) DZConfig.COPPER_HELMET_ENCHANTABILITY.get();
        }
        ...
        public float getToughness() {
            return (Integer) DZConfig.COPPER_HELMET_TOUGHNESS.get();
        }

        @Override
        public float getKnockbackResistance() {
            return (Integer) DZConfig.COPPER_HELMET_KNOCK_BACK_RESISTANCE.get();
        }
    ...
    public boolean HitByEntity(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        stack.damageItem((Integer)DZConfig.COPPER_HELMET_HIT_COST.get(), attacker, (playerEntity) -> {
    ...
    public void addInformation(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        tooltip.add(new StringTextComponent("\u00A76" + "Bad durability even?" + "\u00A76"));
    }
}
