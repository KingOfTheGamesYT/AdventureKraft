...
public class EmeraldHelmet extends ArmorItem {
    private static IArmorMaterial iArmorMaterial = new IArmorMaterial() {
        ...
        public int getDurability(EquipmentSlotType slotIn) {
            return (Integer) DZConfig.EMERALD_HELMET_DURABILITY.get();
        }

        @Override
        public int getDamageReductionAmount(EquipmentSlotType slotIn) {
            return (Integer) DZConfig.EMERALD_HELMET_ARMOR.get();
        }

        @Override
        public int getEnchantability() {
            return (Integer) DZConfig.EMERALD_HELMET_ENCHANTABILITY.get();
        }
        ...
        public float getToughness() {
            return (Integer) DZConfig.EMERALD_HELMET_TOUGHNESS.get();
        }

        @Override
        public float getKnockbackResistance() {
            return (Integer) DZConfig.EMERALD_HELMET_KNOCK_BACK_RESISTANCE.get();
        }
    ...
    public boolean HitByEntity(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        stack.damageItem((Integer)DZConfig.EMERALD_HELMET_HIT_COST.get(), attacker, (playerEntity) -> {
    ...
    public void addInformation(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        tooltip.add(new StringTextComponent("\u00A72" + "Show those greedy villagers how to head bang" + "\u00A72"));
    }
}
