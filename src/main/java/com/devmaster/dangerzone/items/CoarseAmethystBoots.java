...
public class CoarseAmethystBoots extends ArmorItem {
    private static IArmorMaterial iArmorMaterial = new IArmorMaterial() {

        public int getDurability(EquipmentSlotType slotIn) {
            return (Integer) DZConfig.COARSE_AMETHYST_BOOTS_DURABILTY.get();
        }

        @Override
        public int getDamageReductionAmount(EquipmentSlotType slotIn) {
            return (Integer)DZConfig.COARSE_AMETHYST_BOOTS_ARMOR.get();

        }

        @Override
        public int getEnchantability() {
            return (Integer)DZConfig.COARSE_AMETHYST_BOOTS_ENCHANTABILITY.get();
        ...
        public float getToughness() {
            return (Integer)DZConfig.COARSE_AMETHYST_BOOTS_TOUGHNESS.get();
        }

        @Override
        public float getKnockbackResistance() {
            return (Integer)DZConfig.COARSE_AMETHYST_BOOTS_KNOCK_BACK_RESISTANCE.get();
        }

    };

    public CoarseAmethystBoots() {
    ...
    public void addInformation(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
            tooltip.add(new StringTextComponent("\u00A75" + "Sparkling" + "\u00A75"));
        }
    }
