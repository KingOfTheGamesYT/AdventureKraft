...
public class TitaniumChestplate extends ArmorItem {
    private static IArmorMaterial iArmorMaterial = new IArmorMaterial() {
        ...
        public int getDurability(EquipmentSlotType slotIn) {
            return (Integer) DZConfig.TITANIUM_CHESTPLATE_DURABILITY.get();
        }

        @Override
        public int getDamageReductionAmount(EquipmentSlotType slotIn) {
            return (Integer) DZConfig.TITANIUM_CHESTPLATE_ARMOR.get();
        }

        @Override
        public int getEnchantability() {
            return (Integer) DZConfig.TITANIUM_CHESTPLATE_ENCHANTABILITY.get();
        }
        ...
        public float getToughness() {
            return (Integer) DZConfig.TITANIUM_CHESTPLATE_TOUGHNESS.get();
        }

        @Override
        public float getKnockbackResistance() {
            return (Integer) DZConfig.TITANIUM_CHESTPLATE_KNOCK_BACK_RESISTANCE.get();
        }
    ...
    public boolean HitByEntity(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        stack.damageItem((Integer)DZConfig.TITANIUM_CHESTPLATE_HIT_COST.get(), attacker, (playerEntity) -> {
    ...
    public void addInformation(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        tooltip.add(new StringTextComponent("\u00A73" + "Fire Away" + "\u00A73"));
        }
    }